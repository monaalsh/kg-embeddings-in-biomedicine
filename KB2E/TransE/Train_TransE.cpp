...
private:
    int n,method;
    double res;//loss func value
    double count,count1;//loss func gradient
    double rate,margin;
    double belta;
    ...
    void bfgs()
    {
        ...
        int batchsize = fb_h.size()/nbatches;
            for (int epoch=0; epoch<nepoch; epoch++)
            {
                ...
                FILE* f2 = fopen(("../../../../../Documents/kg_embeddings_data/data/transE/relation2vec_has_indication_transE."+version).c_str(),"w");
                FILE* f3 = fopen(("../../../../../Documents/kg_embeddings_data/data/transE/entity2vec_has_indication_transE."+version).c_str(),"w");
                for (int i=0; i<relation_num; i++)
                {
                    for (int ii=0; ii<n; ii++)
...
void prepare()
{
    FILE* f1 = fopen("../../data/entity2id_has_indication_transX.txt","r");
	FILE* f2 = fopen("../../data/relation2id_has_indication_transX.txt","r");
	int x;
    ...
    FILE* f_kb = fopen("../../data/edgelist_has_indication.txt","r");
...
int main(int argc,char**argv)
{
    ...
    int method = 1;
    int n = 50;
    double rate = 0.01; //Mona:optimal lerning rate as mentioned in transE paper
    double margin = 1;
    int i;
